...
const Settings = () => {
  ...
  const handleClick = (id) => {
    ...
    // Obtem os IDs do localStorage ou inicia um array vazio
    const storedIds = JSON.parse(localStorage.getItem('id_servico_selecionado')) || [];

    // Adiciona ou remove o ID com base na presença no array
    if (storedIds.includes(id)) {
      localStorage.setItem('id_servico_selecionado', JSON.stringify(storedIds.filter((storedId) => storedId !== id)));
    } else {
      localStorage.setItem('id_servico_selecionado', JSON.stringify([...storedIds, id]));
    }
  };
  ...
  useEffect(() => {
    // Função para verificar localStorage e atualizar estado
  ...
  const enviar_dados = () => {
  const selectedServiceIds = JSON.parse(localStorage.getItem('id_servico_selecionado')) || [];
